						...
						<li key={route.id}>
                            <Link href={`cities_routes/${route.id}`} className="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-4xl hover:bg-gray-100">
                                ...
                                <div class="flex flex-col justify-between p-4 leading-normal">
                                    ...
                                    <p class="mb-3 font-normal text-gray-700">{route.route_type} {route.status ? <span class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded border border-green-400">Activa</span> : <span class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded">Inactiva</span>}</p>
                                </div>
                            </Link>
                        </li>
					))}
